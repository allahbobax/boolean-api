...
router.get('/:id', async (req, res) => {
  ...
  `;
    ...
    return res.json({ success: true, data: (0, userMapper_1.mapUserFromDb)(result[0]) });
});
// Функции валидации
...
function validateEmail(email) {
    return validator_1.default.isEmail(email) && email.length <= 254; // RFC 5321 limit
}
...
router.patch('/:id', async (req, res) => {
    ...
    for (const [key, value] of Object.entries(updates)) {
        if (ALLOWED_UPDATE_FIELDS[key]) {
            ...
            safeUpdates[key] = processedValue;
        }
    }
